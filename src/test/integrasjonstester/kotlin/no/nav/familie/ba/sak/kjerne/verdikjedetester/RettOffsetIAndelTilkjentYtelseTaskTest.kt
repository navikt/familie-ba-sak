...
import no.nav.familie.ba.sak.kjerne.beregning.RettOffsetIAndelTilkjentYtelseDto
import no.nav.familie.ba.sak.kjerne.beregning.RettOffsetIAndelTilkjentYtelseTask
...
internal class RettOffsetIAndelTilkjentYtelseTaskTest(
    ...
    fun køyrTest() {
        ...
        ).isEqualTo(listOf(1L, 2L, 3L, 4L, 4L, 5L))
        ...
        assertThat(
            andelTilkjentYtelseRepository.finnAndelerTilkjentYtelseForBehandling(behandlingId = behandling2)
                .sortedBy { it.periodeOffset }
                .map { it.periodeOffset }
        ).isEqualTo(listOf(0L, 1L, 2L, 3L, 4L, 5L))
    }
