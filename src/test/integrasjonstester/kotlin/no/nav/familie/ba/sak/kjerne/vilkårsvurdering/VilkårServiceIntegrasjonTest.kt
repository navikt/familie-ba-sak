...
class VilkårServiceIntegrasjonTest(
        ...
        vilkårsvurdering.personResultater.forEach { personResultat ->
            personResultat.vilkårResultater.forEach { vilkårResultat ->
                when (vilkårResultat.vilkårType) {
                    ...
                    else -> assertFalse(vilkårResultat.erAutomatiskVurdert, "Vilkår ${vilkårResultat.vilkårType} skal ikke være automatisk vurdert")
                }
            }
        }
    }
    ...
    fun `Endring på automatisk vurderte vilkår(manuell vilkårsvurdering) skal settes til manuell ved endring`() {
        ...
        val endretUnder18ÅrVilkårForBarn =
            endretVilkårsvurdering
                .find { it.personIdent == barnFnr }
                ?.vilkårResultater
                ?.find { it.vilkårType == Vilkår.UNDER_18_ÅR }
    ...
    fun `Vilkårsvurdering kopieres riktig`() {
        ...
        assertEquals(
            1,
            kopiertVilkårsvurdering.personResultater
                .first()
                .andreVurderinger.size,
        )
        assertEquals(
    ...
    fun `skal lage utvidet barnetrygd vilkår for migreringsbehandling`() {
        ...
        assertTrue {
            vilkårsvurdering.personResultater
                .find { it.aktør.aktivFødselsnummer() == fnr }!!
                .vilkårResultater.size == 2
        }
    ...
    fun `skal kunne legge til utvidet barnetrygd vilkår for ordinær behandling dersom det er utvidet på vilkårsvurderingen allerede`() {
        ...
        val utvidetVilkår =
            vilkårsvurdering.personResultater
                .find { it.erSøkersResultater() }
                ?.vilkårResultater
                ?.single { it.vilkårType == Vilkår.UTVIDET_BARNETRYGD }
        vilkårService.endreVilkår(
            behandlingId = behandling.id,
    ...
    fun `skal ikke slette utvidet barnetrygd vilkår for migreringsbehandling når det finnes i forrige behandling`() {
        ...
        assertTrue {
            vilkårsvurdering.personResultater
                .first { it.aktør.aktivFødselsnummer() == fnr }
                .vilkårResultater.size == 3
        }
        val exception =
            assertThrows<RuntimeException> {
    ...
    fun `skal slette utvidet barnetrygd vilkår for migreringsbehandling`() {
        ...
        assertTrue {
            vilkårsvurderingEtterSlett
                ...
                .first { it.aktør.aktivFødselsnummer() == fnr }
                .vilkårResultater
                .none { it.vilkårType == Vilkår.UTVIDET_BARNETRYGD }
        }
    }
    ...
    fun Vilkår.påvirketVilkårForEndreMigreringsdato() =
        this in
            listOf(
