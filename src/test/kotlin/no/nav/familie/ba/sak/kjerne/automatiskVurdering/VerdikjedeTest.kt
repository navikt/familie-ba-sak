...
class Integrasjonstest(
    ...
    fun `sjekk om mor har saker men ikke løpende utbetalinger i Infotrygd`() {
        every { infotrygdService.harLøpendeSakIInfotrygd(mutableListOf(enMorsIdent)) } returns false
        every { infotrygdService.harÅpenSakIInfotrygd(mutableListOf(enMorsIdent)) } returns true


        assertEquals(true, velgFagSystemService.morHarSakerMenIkkeLøpendeIInfotrygd(enMorsIdent))
    }

    @Test
    fun `sjekk om mor har løpende utbetalinger i infotrygd`() {
        every { infotrygdService.harLøpendeSakIInfotrygd(mutableListOf(enMorsIdent)) } returns true

        assertEquals(true, velgFagSystemService.morHarLøpendeSakIInfotrygd(enMorsIdent))
    }
