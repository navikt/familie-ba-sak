...
class OpplysningspliktTest(
    ...
    @Test
    fun `Skal opprette opplysningsplikt-vilkår på søker når 'innhente opplysninger'-brev sendes ut og IKKE ta med hjemmel 17 og 18 i vedtaksbrev når opplysningsplikt-vilkåret er oppfylt`() {
        val scenario = mockServerKlient().lagScenario(
            RestScenario(
                søker = RestScenarioPerson(fødselsdato = "1990-04-20", fornavn = "Mor", etternavn = "Søker"),
                barna = listOf(
        ...
        // Sette opplysningsplikt vilkåret til oppfylt og sjekke at hjemlene blir riktig
        opplysningspliktVilkårPåSøker?.resultat = Resultat.OPPFYLT
        vilkårsvurderingService.oppdater(vilkårsvurdering = vilkårsvurdering!!)

        val vilkårsvurderingOppdatert = vilkårsvurderingService.hentAktivForBehandling(behandlingId = behandling.id)

        Assertions.assertTrue(vilkårsvurderingOppdatert?.personResultater?.single { it.erSøkersResultater() }?.andreVurderinger?.single { it.type == AnnenVurderingType.OPPLYSNINGSPLIKT }?.resultat == Resultat.OPPFYLT)
        ...
        Assertions.assertFalse(hjemmeltekst.contains("17"))
        Assertions.assertFalse(hjemmeltekst.contains("18"))
    }

    @Test
    fun `Skal opprette opplysningsplikt-vilkår på søker når 'varsel om revurdering'-brev sendes ut og ta med hjemmel 17 og 18 i vedtaksbrev når opplysningsplikt-vilkåret ikke er oppfylt`() {
        val scenario = mockServerKlient().lagScenario(
            RestScenario(
                søker = RestScenarioPerson(fødselsdato = "1990-04-20", fornavn = "Mor", etternavn = "Søker"),
                barna = listOf(
        ...
        // Send "varsel om revurdering"-brev og sjekk at opplysningsplikt vilkåret dukker opp på _kun_ søker
        dokumentService.sendManueltBrev(
            fagsakId = behandling.fagsak.id,
            manueltBrevRequest = ManueltBrevRequest(
                brevmal = BrevType.VARSEL_OM_REVURDERING,
                mottakerIdent = scenario.søker.ident,
                enhet = Enhet(enhetId = "1234", enhetNavn = "Enhet Enhetesen")
            ),
            behandling = behandling
    ...
    @Test
    fun `Skal opprette opplysningsplikt-vilkår på søker når 'varsel om revurdering'-brev sendes ut og IKKE ta med hjemmel 17 og 18 i vedtaksbrev når opplysningsplikt-vilkåret er oppfylt`() {
        val scenario = mockServerKlient().lagScenario(
            RestScenario(
                søker = RestScenarioPerson(fødselsdato = "1990-04-20", fornavn = "Mor", etternavn = "Søker"),
                barna = listOf(
