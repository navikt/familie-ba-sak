...
class RolletilgangTest(
    ...
    fun `Skal kaste feil når innlogget veileder prøver å opprette fagsak gjennom rest-endepunkt`() {
        ...
        assertEquals("Mock McMockface med rolle VEILEDER har ikke tilgang til å opprette fagsak. Krever SAKSBEHANDLER.", ressurs.melding)
    }

    @Test
    fun `Skal få 201 når innlogget saksbehandler prøver å opprette fagsak gjennom rest-endepunkt`() {
        val fnr = randomFnr()
        ...
        val response = restTemplate.postForEntity<Ressurs<Fagsak>>(hentUrl("/api/fagsaker"), requestEnty)
    ...
    fun `Skal kaste feil når innlogget veileder prøver å opprette behandling gjennom test-rest-endepunkt som validerer på db-nivå`() {
        ...
        val requestEnty = HttpEntity<String>(objectMapper.writeValueAsString(nyOrdinærBehandling(fnr)), header)
