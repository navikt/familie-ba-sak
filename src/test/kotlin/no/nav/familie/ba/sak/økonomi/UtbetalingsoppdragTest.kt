...
internal class UtbetalingsoppdragPeriodiseringTest {
    ...
    fun `skal opprette et nytt utbetalingsoppdrag med løpende periodeId fordelt på flere klasser`() {
        ...
        assertUtbetalingsperiode(utbetalingsperioderPerKlasse["BATRSMA"]!![0], id + 0, null, 660, "2018-07-01", "2021-06-30")
        assertUtbetalingsperiode(utbetalingsperioderPerKlasse["BATRSMA"]!![1], id + 1, id + 0, 660, "2020-04-01", "2023-03-31")

        assertUtbetalingsperiode(utbetalingsperioderPerKlasse["BATR"]!![0], id + 0, null, 1054, "2020-03-01", "2038-02-28")
        assertUtbetalingsperiode(utbetalingsperioderPerKlasse["BATR"]!![1], id + 1, id + 0, 1054, "2020-05-01", "2021-02-28")
    }
    ...
    fun `skal ikke tillate for stor offset på periodeId`() {
        ...
        utbetalingsperiodeMal.lagPeriodeFraAndel("A", segment.fom, segment.tom, segment.value, 999) //OK

        assertThrows<IllegalArgumentException> {
            utbetalingsperiodeMal.lagPeriodeFraAndel("A", segment.fom, segment.tom, segment.value, 1000) //OK
        }
    }
