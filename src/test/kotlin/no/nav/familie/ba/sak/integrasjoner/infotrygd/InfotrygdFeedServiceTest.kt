...
class InfotrygdFeedServiceTest : AbstractSpringIntegrationTestDev() {
    ...
    fun `Skal ikke send start behandling feed hvis feature er togglet av`() {
        ...
        every { featureToggleServiceMock.isEnabled(FeatureToggleConfig.SEND_START_BEHANDLING_TIL_INFOTRYGD, any()) } returns false
    ...
    fun `Skal send riktig start behandling feed hvis feature er togglet`() {
        ...
        every { featureToggleServiceMock.isEnabled(FeatureToggleConfig.SEND_START_BEHANDLING_TIL_INFOTRYGD, any()) } returns true
        val ident = "123"
