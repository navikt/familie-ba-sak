...
class UtbetalingsoppdragIntegrasjonTest(
    ...
    fun `skal opprette et nytt utbetalingsoppdrag med felles løpende periodeId og separat kjeding på to personer`() {
        ...
        val fagsak = fagsakService.hentEllerOpprettFagsakForPersonIdent(personMedFlerePerioder.aktør.aktivIdent())
        ...
        val andelerTilkjentYtelse = listOf(
            lagAndelTilkjentYtelse(
                ...
                aktør = personidentService.hentOgLagreAktør(personMedFlerePerioder.aktør.aktivIdent()),
            ...
            lagAndelTilkjentYtelse(
                ...
                aktør = personidentService.hentOgLagreAktør(tilfeldigPerson.aktør.aktivIdent()),
            )
        )
    ...
    fun `skal opprette revurdering med endring på eksisterende periode`() {
        ...
        val andelerFørstegangsbehandling = listOf(
            lagAndelTilkjentYtelse(
                ...
                periodeIdOffset = 0,
                person = person,
                aktør = personidentService.hentOgLagreAktør(person.aktør.aktivIdent()),
                tilkjentYtelse = tilkjentYtelse
            ),
            lagAndelTilkjentYtelse(
    ...
    fun `Skal opprette revurdering med nytt barn`() {
        ...
        val andelerRevurdering = listOf(
            lagAndelTilkjentYtelse(
                ...
                periodeIdOffset = 2,
                person = nyPerson,
                aktør = personidentService.hentOgLagreAktør(nyPerson.aktør.aktivIdent()),
                tilkjentYtelse = tilkjentYtelse2
            ),
            lagAndelTilkjentYtelse(
                årMnd("2037-01"),
    ...
    fun `skal opprette et nytt utbetalingsoppdrag med to andeler på samme person og separat kjeding for småbarnstillegg`() {
        val personMedFlerePerioder = tilfeldigPerson()
        val fagsak = fagsakService.hentEllerOpprettFagsakForPersonIdent(personMedFlerePerioder.aktør.aktørId)
        val behandling = behandlingService.lagreNyOgDeaktiverGammelBehandling(lagBehandling(fagsak))
        val vedtak = lagVedtak(behandling = behandling)
        val andelerTilkjentYtelse = listOf(
            lagAndelTilkjentYtelse(
    ...
    fun `Ved full betalingsoppdrag skal komplett utbetalinsoppdrag genereres også når ingen endring blitt gjort`() {
        ...
        val andelerFørstegangsbehandling = listOf(
            lagAndelTilkjentYtelse(
                ...
                periodeIdOffset = 0,
                person = person,
                aktør = personidentService.hentOgLagreAktør(person.aktør.aktørId),
                tilkjentYtelse = tilkjentYtelse
            ),
            lagAndelTilkjentYtelse(
                årMnd("2030-01"),
