...
class YtelsePersonKravTest {
    ...
    @Test
    fun `Skal finne tidligere ytelseperson`() {
        val søknadDTO = lagSøknadDTO(
                søkerIdent = søker.personIdent.ident,
        ...
        assertEquals(2, ytelsePersoner.size)
        assertTrue(ytelsePersoner.any { it.personIdent == barn1.personIdent.ident && it.ytelseType == YtelseType.ORDINÆR_BARNETRYGD && it.kravOpprinnelse == KravOpprinnelse.TIDLIGERE })
        assertTrue(ytelsePersoner.any { it.personIdent == barn2.personIdent.ident && it.ytelseType == YtelseType.ORDINÆR_BARNETRYGD && it.kravOpprinnelse == KravOpprinnelse.SØKNAD })
    }
}