...
class PreutfyllBosattIRiketServiceTest {
    ...
    @Test
    fun `skal gi oppfylt bosatt i riket vilkåer med begrunnelse 'Norsk bostedsadresse i minst 6 måneder' dersom bosatt i Norge i minst 6 mnd og behandlingsårsak er fødselshendelse`() {
        // Arrange
        val behandling = lagBehandling(årsak = BehandlingÅrsak.FØDSELSHENDELSE)
        val barnAktør = lagAktør()
        val søkerPersonIdent = randomFnr()
                        ...
                        it.bostedsadresser =
                            mutableListOf(
                                lagGrVegadresseBostedsadresse(
                                    periode =
                                        DatoIntervallEntitet(
                                            fom = LocalDate.now().minusMonths(6).minusDays(1),
                                        ),
                                    matrikkelId = 12345L,
                                ),
                            )
        ...
        assertThat(vilkårResultat!!.resultat).isEqualTo(Resultat.OPPFYLT)
        assertThat(vilkårResultat.begrunnelse).isEqualTo(PREUTFYLT_VILKÅR_BEGRUNNELSE_OVERSKRIFT + "- Norsk bostedsadresse i minst 6 måneder.")
