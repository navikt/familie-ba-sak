...
class ValutakursControllerTest {
    ...
    private val restValutakurs: RestValutakurs = RestValutakurs(1, YearMonth.of(2020, 1), null, listOf(barnId), null, null, null, UtfyltStatus.OK)
    ...
    fun `Test at valutakurs hentes fra ECB dersom dato og valuta er satt`() {
        ...
        val valuta = "SEK"
        assertThrows<Exception> { valutakursController.oppdaterValutakurs(1, restValutakurs.copy(valutakursdato = valutakursDato, valutakode = valuta)) }
    ...
    fun `Test at valutakurs ikke hentes fra ECB dersom valuta ikke er satt`() {
        val valutakursDato = LocalDate.of(2022, 1, 1)
        assertThrows<Exception> { valutakursController.oppdaterValutakurs(1, restValutakurs.copy(valutakursdato = valutakursDato)) }
        verify(exactly = 0) { ecbService.hentValutakurs("SEK", valutakursDato) }
        verify(exactly = 1) { valutakursService.oppdaterValutakurs(any(), any()) }
    }
    ...
    fun `Test at valutakurs ikke hentes fra ECB dersom ISK og før 1 feb 2018`() {
        ...
        assertThrows<Exception> { valutakursController.oppdaterValutakurs(1, restValutakurs.copy(valutakursdato = valutakursDato, valutakode = "ISK")) }
