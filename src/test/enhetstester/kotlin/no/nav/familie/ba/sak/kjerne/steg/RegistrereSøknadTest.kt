...
class RegistrereSøknadTest {
    ...
    inner class UtførStegOgAngiNesteTest {
        ...
        fun `skal registrer søknad uten å oppdatere behandlingstema`() {
            ...
            // Assert
            verify(exactly = 0) { behandlingstemaService.oppdaterBehandlingstemaForRegistrerSøknad(any(), any()) }
        ...
        fun `skal registrer søknad og oppdatere behandlingstema`() {
            ...
            every { behandlingstemaService.oppdaterBehandlingstemaForRegistrerSøknad(behandling, søknadDTO.underkategori.tilDomene()) } answers {
            ...
            // Assert
            verify(exactly = 1) { behandlingstemaService.oppdaterBehandlingstemaForRegistrerSøknad(behandling, søknadDTO.underkategori.tilDomene()) }
            verify(exactly = 1) { loggService.opprettRegistrertSøknadLogg(behandling, false) }
            verify(exactly = 1) { søknadGrunnlagService.lagreOgDeaktiverGammel(any()) }
            verify(exactly = 1) { persongrunnlagService.registrerBarnFraSøknad(søknadDTO, behandling, null) }
            verify(exactly = 1) { tilbakestillBehandlingService.initierOgSettBehandlingTilVilkårsvurdering(behandling, restRegistrerSøknad.bekreftEndringerViaFrontend) }
