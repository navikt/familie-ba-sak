 ...
 */
fun Tidslinje<VilkårRegelverkResultat, Dag>.tilMånedEtterVilkårsregler() = tidslinje {
    (fraOgMed().tilForrigeMåned()..tilOgMed().tilNesteMåned())
        .map { måned ->
            val resultatSisteDagForrigeMåned = innholdForTidspunkt(måned.forrige().tilSisteDagIMåneden())
            val resultatFørsteDagDenneMåned = innholdForTidspunkt(måned.tilFørsteDagIMåneden())

            if (resultatSisteDagForrigeMåned.erOppfylt() && resultatFørsteDagDenneMåned.erOppfylt()) {
                måned.tilPeriodeMedInnhold(resultatFørsteDagDenneMåned)
            } else {
