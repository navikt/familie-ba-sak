...
import no.nav.familie.ba.sak.config.BehandlerRolle.BESLUTTER
import no.nav.familie.ba.sak.config.BehandlerRolle.FORVALTER
...
class LoggService(
    ...
    fun opprettBehandlendeEnhetEndret(
        ...
        lagre(
            Logg(
                ...
                tekst =
                    "Behandlende enhet ${if (manuellOppdatering) "manuelt" else "automatisk"} endret fra " +
    ...
    private fun opprettLivshendelseLogg(
        behandling: BehandlingLoggRequest,
        tittel: String,
    ) {
        lagre(
            Logg(
    ...
    fun opprettSendTilBeslutterLogg(
        behandling: Behandling,
        skalAutomatiskBesluttes: Boolean,
    ) {
        lagre(
            Logg(
                behandlingId = behandling.id,
    ...
    fun opprettBeslutningOmVedtakLogg(
        ...
        lagre(
            Logg(
                ...
                opprettetAv =
                    if (behandlingErManuellMigreringSomBleAutomatiskBesluttet) {
    ...
    fun opprettOppdaterVentingLogg(
        ...
        val tekst =
            if (endretFrist != null && endretÅrsak != null) {
    ...
    fun loggManueltRegistrertDødsfallDato(
        behandlingId: BehandlingId,
        person: Person,
        begrunnelse: String,
    ) = lagre(
