...
class VedtakService(
    ...
    fun lagBrevBegrunnelseForUtbetalingEllerOpphør(
        ...
        return if (VedtakBegrunnelseUtils.vedtakBegrunnelserIkkeTilknyttetVilkår.contains(vedtakBegrunnelse)) {
            ...
            if (personerMedUtgjørendeVilkårForUtbetalingsperiode.isEmpty()) {
                throw FunksjonellFeil(
                    melding = "Begrunnelsen samsvarte ikke med vilkårsvurderingen",
    ...
    @Transactional
    fun settFritekstbegrunnelserPåVedtaksperiodeOgType(
        restPostFritekstVedtakBegrunnelser: RestPostFritekstVedtakBegrunnelser,
        fagsakId: Long,
        validerKombinasjoner: Boolean = true
    ): Vedtak {
        val vedtak = hentVedtakForAktivBehandling(fagsakId)
    ...
    @Transactional
    fun slettBegrunnelserForPeriodeOgVedtaksbegrunnelseTyper(
    ...
    @Transactional
    fun slettAvslagBegrunnelserForVilkår(
    ...
    @Transactional
    fun oppdaterAvslagBegrunnelserForVilkår(
                ...
                VedtakBegrunnelse(
                    ...
                    brevBegrunnelse = it.begrunnelse.hentBeskrivelse(
                        ...
                        barnasFødselsdatoer = if (personDetGjelder.type == PersonType.BARN) listOf(
                            personDetGjelder.fødselsdato
                        ) else emptyList(),
                        månedOgÅrBegrunnelsenGjelderFor =
                        VedtakBegrunnelseType.AVSLAG.hentMånedOgÅrForBegrunnelse(
                            Periode(
    ...
    companion object {
        ...
        fun mapTilRestAvslagBegrunnelser(
            avslagBegrunnelser: List<VedtakBegrunnelse>,
            personopplysningGrunnlag: PersonopplysningGrunnlag
        ): List<RestAvslagBegrunnelser> =
            mapAvslagBegrunnelser(
                avslagBegrunnelser = avslagBegrunnelser.filter { it.begrunnelse != VedtakBegrunnelseSpesifikasjon.AVSLAG_FRITEKST },
                personopplysningGrunnlag = personopplysningGrunnlag
            )
         ...
         */
        fun mapAvslagBegrunnelser(
            avslagBegrunnelser: List<VedtakBegrunnelse>,
            personopplysningGrunnlag: PersonopplysningGrunnlag
        ): Map<NullablePeriode, List<String>> {
                            ...
                            if (fellesBegrunnelse == VedtakBegrunnelseSpesifikasjon.AVSLAG_UREGISTRERT_BARN) {
                                ...
                                BrevtekstParametre(
                                    ...
                                    månedOgÅrBegrunnelsenGjelderFor = VedtakBegrunnelseType.AVSLAG.hentMånedOgÅrForBegrunnelse(
                                        Periode(
                                            tilfellerForSammenslåing.first().vilkårResultat?.periodeFom
                                                ?: TIDENES_MORGEN,
