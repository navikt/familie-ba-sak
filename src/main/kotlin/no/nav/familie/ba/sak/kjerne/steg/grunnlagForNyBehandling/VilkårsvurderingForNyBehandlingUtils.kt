...
data class VilkårsvurderingForNyBehandlingUtils(
        ...
        Vilkårsvurdering(behandling = behandling).apply {
            when {
                behandling.opprettetÅrsak == BehandlingÅrsak.FØDSELSHENDELSE -> {
                    personResultater =
                        lagPersonResultaterForFødselshendelse(
    ...
    private fun lagPersonResultaterForFødselshendelse(
        ...
        return personopplysningGrunnlag.søkerOgBarn
            .map { person ->
    ...
    fun lagPersonResultaterForMigreringsbehandlingMedÅrsakEndreMigreringsdato(
        ...
        personopplysningGrunnlag.søkerOgBarn
            .map { person ->
                ...
                val oppfylteVilkårResultaterForPerson =
                ...
                val kopierteVilkårResultater =
                    oppfylteVilkårResultaterForPerson
                        .map { oppfyltVilkårResultat ->
...
fun førstegangskjøringAvVilkårsvurdering(aktivVilkårsvurdering: Vilkårsvurdering?): Boolean = aktivVilkårsvurdering == null

fun finnAktørerMedUtvidetFraAndeler(andeler: List<AndelTilkjentYtelse>): List<Aktør> = andeler.filter { it.erUtvidet() }.map { it.aktør }
