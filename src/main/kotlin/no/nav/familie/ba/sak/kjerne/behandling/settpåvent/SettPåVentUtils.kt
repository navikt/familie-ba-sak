...
fun validerBehandlingKanSettesPåVent(
    ...
    if (gammelSettPåVent != null) {
        throw FunksjonellFeil(
            melding = "Behandling ${behandling.behandlingId.id} er allerede satt på vent.",
    ...
    if (frist.isBefore(LocalDate.now())) {
        throw FunksjonellFeil(
            melding = "Frist for å vente på behandling ${behandling.behandlingId.id} er satt før dagens dato.",
    ...
    if (behandling.status == BehandlingStatus.AVSLUTTET) {
        throw FunksjonellFeil(
            melding = "Behandling ${behandling.behandlingId.id} er avsluttet og kan ikke settes på vent.",
    ...
    if (!behandling.aktiv) {
        throw Feil(
            "Behandling ${behandling.behandlingId.id} er ikke aktiv og kan ikke settes på vent."
        )
    }
}
