...
enum class VedtakBegrunnelseSpesifikasjon(val tittel: String, val erTilgjengeligFrontend: Boolean = true) : IVedtakBegrunnelse {
    ...
    INNVILGET_OMSORG_FOR_BARN("Adopsjon, surrogati: Omsorgen for barn") {
        ...
        override fun hentBeskrivelse(
            ...
            Målform.NB -> "${innvilgetFormulering(gjelderSøker, barnasFødselsdatoer, målform)}" +
                          "fordi du har omsorgen for ${barnasFødselsdatoer.barnetBarnaFormulering()} fra $månedOgÅrBegrunnelsenGjelderFor."
            Målform.NN -> "${innvilgetFormulering(gjelderSøker, barnasFødselsdatoer, målform)}" +
                          "fordi du har omsorga for ${barnasFødselsdatoer.barnetBarnaFormulering()} frå $månedOgÅrBegrunnelsenGjelderFor."
        }
    },
    ...
    REDUKSJON_MANGLENDE_OPPLYSNINGER(tittel = "Ikke mottatt opplysninger", erTilgjengeligFrontend = true) {
    ...
    REDUKSJON_UNDER_6_ÅR("Barn 6 år") {
        ...
        override fun hentBeskrivelse(
                ...
                when (målform) {
                    Målform.NB -> "Barnetrygden reduseres fordi barn født ${barnasFødselsdatoer.tilBrevTekst()} er 6 år."
                    Målform.NN -> "Barnetrygda er redusert fordi barn fødd ${barnasFødselsdatoer.tilBrevTekst()} er 6 år."
                }
    },
    ...
    OPPHØR_BARN_DØD(tittel = "Et barn død", erTilgjengeligFrontend = true) {
    ...
    OPPHØR_FLERE_BARN_DØD(tittel = "Flere barn er døde", erTilgjengeligFrontend = true) {
