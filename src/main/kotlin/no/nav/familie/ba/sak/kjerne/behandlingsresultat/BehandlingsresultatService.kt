...
class BehandlingsresultatService(
    ...
    internal fun finnPersonerFremstiltKravFor(
        ...
        val personerFremstiltKravFor = when {
            ...
            behandling.opprettetÅrsak == BehandlingÅrsak.FØDSELSHENDELSE -> persongrunnlagService.finnNyeBarn(
                behandling,
                forrigeBehandling,
            ).map { it.aktør }

            behandling.erManuellMigrering() || behandling.opprettetÅrsak == BehandlingÅrsak.KLAGE || behandling.erSatsendring() -> persongrunnlagService.hentAktivThrows(
                behandling.id,
