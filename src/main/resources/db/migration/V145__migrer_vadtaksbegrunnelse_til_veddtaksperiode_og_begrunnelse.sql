INSERT INTO vedtaksperiode (id, fk_vedtak_id, fom, tom, type)
    (SELECT NEXTVAL('vedtaksperiode_seq'),
            fk_vedtak_id,
            fom,
            tom,
     ...
     WHERE NOT EXISTS(SELECT id
                      ...
                      WHERE v.fk_vedtak_id = vb.fk_vedtak_id
                        AND ((v.fom IS NULL AND vb.fom IS NULL) OR v.fom = vb.fom)
                        AND ((v.tom IS NULL AND vb.tom IS NULL) OR v.tom = vb.tom)
                        AND v.type = v.type)
