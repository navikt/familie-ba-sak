with ny_stonad_tom (tilkjent_ytelse_id, ny_stonad_tom) as (select ty.id, max(aty.stonad_tom)
...
with ny_stonad_fom (tilkjent_ytelse_id, ny_stonad_fom) as (select ty.id, min(aty.stonad_fom)
                                                           from andel_tilkjent_ytelse aty
                                                                    join tilkjent_ytelse ty on ty.id = aty.tilkjent_ytelse_id
