...
select distinct
    on (person_ident) person_ident,
                      ...
                      case
                          when
                                  aktoer_id IN (select y.aktoer_id
                                                from po_person i
                                                         join po_person y on i.aktoer_id = y.aktoer_id
                                                where y.person_ident != i.person_ident) then false
                          else true
                          end,
                      null,
                      opprettet_av,
...
alter table FAGSAK_PERSON
    rename column AKTOER_ID to FK_AKTOER_ID;
alter table FAGSAK_PERSON
...
alter table fagsak
    add column fk_aktoer_id varchar;
alter table FAGSAK
